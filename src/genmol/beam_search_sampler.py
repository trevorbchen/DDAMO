...
def decode_smiles(model, x, fix=True):
    ...
    BracketSAFE is an alternative encoding that wraps SAFE fragments in
    brackets; the model config flag determines which decoder path to use.
    """
    ...
    for s in strings:
        try:
            ...
            smi = None
        if smi:
            # SAFE decoding can produce disconnected fragments (dot-separated);
            # keep only the largest fragment by string length.
...
class QEDForwardOp:
    """Default reward: RDKit QED score in [0, 1]."""
