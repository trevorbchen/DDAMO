...
python3 run_generation.py sampler=beam_search num_samples=5 output_dir="$OUT" \
    hydra.run.dir="$OUT/beam_search"

echo ""
echo "=== MCTS (5 samples) ==="
python3 run_generation.py sampler=mcts num_samples=5 output_dir="$OUT" \
    hydra.run.dir="$OUT/mcts"

echo ""
echo "=== Running evals on beam search output ==="
# Evals expect 'sequence' column, run_generation.py outputs 'smiles' — rename
